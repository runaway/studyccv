...
static void 
_ccv_filter_direct_8u(ccv_dense_matrix_t* a, 
					  ...
					  int padding_pattern)
{
	...
	// 创建能装下a,b的稠密矩阵pa	
	ccv_dense_matrix_t* pa = 
		ccv_dense_matrix_new(a->rows + b->rows / 2 * 2, 
							 a->cols + b->cols / 2 * 2, 
							 CCV_8U | CCV_C1, 0, 0);
	...
	for (i = 0; i < pa->rows; i++)
	{
		for (j = 0; j < pa->cols; j++)
		{
			// 将a填充到pa中间，剩余的空间用a的边界值填充	
			pa->data.u8[i * pa->step + j] = 
				a->data.u8[ccv_clamp(i - b->rows / 2, 0, a->rows - 1) * a->step 
						 + ccv_clamp(j - b->cols / 2, 0, a->cols - 1)];
		}
	}
	...
	else // 如果b的非零元素大于等于元素总数的0.75倍
	{
		...
		for (i = 0; i < d->rows; i++)
		{
			for (j = 0; j < d->cols; j++)
			{
				...
				// 将卷积结果右移14位再做饱和运算到m_ptr == d->data.u8
				m_ptr[j] = ccv_clamp(z >> 14, 0, 255);
			}
...
// hog, root_classifier->root.w, &response, 0, CCV_NO_PADDING
...
/*
	将可变形模型与图片进行匹配是一个复杂的优化问题。局部搜索方法需要在正确的位
置附近进行初始化[2][7][43]。为了保证全局最优匹配，需要更激进的搜索方法。其中一
个流行的部件模型的搜索方法是将部件的位置约束在一个小的可能位置集合中，此位置集
合是根据兴趣点检测得到的[1][18][42]。星型(树型)图结构模型[9][15][19]允许使用动
...
ccv_distance_transform(ccv_dense_matrix_t* a, 
					   ccv_dense_matrix_t** b, 
					   int type, 
					   ccv_dense_matrix_t** x, int x_type, 
					   ccv_dense_matrix_t** y, int y_type, 
					   double dx, double dy, double dxx, double dyy, 
					   int flag)
{
