...
/*
ÓÃÖ¸¶¨µÄ²ÎÊı´´½¨Ò»¸öĞÂµÄÉî¶È¾í»ıÍøÂç
					   ...
					   int output) // ¶ÔÓÚ×îºóµÄÈ«Á¬½Ó²ãÖ¸¶¨½ÚµãµÄÊä³öÊıÄ¿
{
	...
	// ÔÚµÚÒ»¸öÈ«Á¬½Ó²ãÖ®ºóµÄËùÓĞ²ã¶¼Ó¦¸ÃÖ»ÊÇÈ«Á¬½Ó²ã
											...
											ccv_dense_matrix_t** b)
{
	...
	// ÉèÖÃaµÄĞĞÊıÎªÔ­ĞĞ¡¢ÁĞ¡¢Í¨µÀÊıµÄ³Ë»ı£¬ÁĞÊıÎª1
	...
	// ¶¨ÒåÄ¿±ê¾ØÕóÆ«ÖÃÊı¾İÖ¸Õëbptr
	float* bptr = db->data.f32;

	// ÉèÖÃÄ¿±ê¾ØÕóÃ¿Ò»ĞĞµÄÆ«ÖÃ
	for (i = 0; i < db->rows; i++)
		bptr[i] = layer->bias[i];
	...
	// Èç¹ûÈ«Á¬½Ó²ãµÄÀàĞÍ±êÖ¾²»ÊÇrelu,0 - ReLU, 1 - no ReLU
	if (layer->net.full_connect.relu)
	{
		for (i = 0; i < db->rows; i++)
		{
			// È·±£Ä¿±ê¾ØÕóÆ«ÖÃµÄÃ¿Ò»ĞĞ²»Ğ¡ÓÚÁã,f(x)=max(0,x)
			bptr[i] = ccv_max(0, bptr[i]); // relu
		}
	}

	// ÉèÖÃÔ´¾ØÕóµÄĞĞ¡¢ÁĞ¡¢ÀàĞÍ¡¢²½³¤
...
// ÓÃ¾í»ıÍøÂçÀ´°ÑÍ¼Ïñ±àÂë³É½ô´Õ±íÊ¾
/*
						...
						int batch)
{
	...
	// ½ô´Õ±íÊ¾µÄÊä³ö¾ØÕócÊı×é
	convnet->acts[convnet->count - 1] = *b;

	// a: ÊäÈëÍ¼ÏñCÊı×é
	// ´ÓÍ¼ÏñÊäÈë²ãµ½µÚ0²ãÕıÏò´«²¥
...
// ÕÒµ½ÕıÔÚÉ¨ÃèµÄ²ã(ËüÊÇ×îºóÒ»¸ö¾í»ı²ã)
...
static int _ccv_convnet_find_scan(ccv_convnet_t* convnet)
{
	...
	{
		// Èç¹û²ãÀàĞÍÎª¾í»ı²ãÔò·µ»Ø²ãĞòºÅ	
						  ...
						  int batch)
{
	...
	for (i = 0; i < batch; i++)
	{
		...
		for (t = 0; t <= !!symmetric; t++)
		{
			...
			// ´ÓµÚ0²ãÑ­»·µ½×îºóÒ»¸ö¾í»ı²ã
			for (j = 0; j < scan + 1; j++)
			{
				ccv_convnet_layer_t* layer = convnet->layers + j;

				// ¸ù¾İ²ãÀàĞÍ¼ÆËãÉèÖÃÊä³öĞĞÁĞÊıºÍ·Ö¸î·½Ê½
			...
			for (k = 0; k < 5; k++)
			{
				...
				// ½«b[scan + 1]°´offsets¶¨ÒåµÄ¼¸¸ö¹Ø¼üµã»®·Öµ½input
				ccv_slice(b[scan + 1], 
						  (ccv_matrix_t**)&input, 
						  CCV_32F, 
						  offsets[k][1], 
		...
		// ÏÖÔÚÔÚcÀïÃæÓĞËùÓĞµÄÁË£¬×ö×îºóµÄÈ«Á¬½Ó´«²¥
		...
		for (j = 0; j < tops; j++)
		{
			...
			{
				// ¼ÇÂ¼Ã¿Ò»ĞĞµÄ×î´óÖµmax_valºÍË÷Òımax_idx	
...
{
	// aÊÇÊäÈëÌİ¶È¾ØÕó(ÓÃÓÚ·´Ïò´«²¥),yÊÇÊä³ö¾ØÕó(ÓÃÓÚÕıÏò´«²¥)
	// xÊÇÊäÈë¾ØÕó(ÓÃÓÚÕıÏò´«²¥),bÊÇÊä³öÌİ¶È¾ØÕó(Ìİ¶È,»ò³ÆÎª´«²¥Ìİ¶ÈÎó²î)
							...
							ccv_convnet_t* update_params)
{
	...
	// ×îºóÒ»²ã±ØĞëÊÇÈ«Á¬½Ó²ãÒÔÉú³Ésoftmax½á¹û
	...
	for (i = 0; i < convnet->count; i++)
	{
		switch (update_params->layers[i].type)
		{
			...
			case CCV_CONVNET_FULL_CONNECT:
			{
				...
				for (j = 0; j < convnet->layers[i].wnum; j++)
				{
					// ¸üĞÂÈ¨ÖØ:ÕâÀïÓÃ×î¼òµ¥µÄ·½·¨À´¸üĞÂ£¬¼´ËùÓĞ²ÎÊı¶¼W = W - learn_rate * gradient
					vw[j] = 
						layer_params[i].w.momentum * vw[j] 
					  - layer_params[i].w.decay * layer_params[i].w.learn_rate * w[j] 
					  + learn_rate * dw[j];
...
1.Ê×ÏÈÊ¹ÓÃËæ»úº¯Êı¶ÔÃ¿Ò»²ã¼äµÄÁ¬½ÓÈ¨Öµ¾ØÕóºÍÆ«ÖÃÏòÁ¿½øĞĞËæ»ú³õÊ¼»¯.
2.ÒÀ´ÎÊ¹ÓÃÒ»¸öÑµÁ·Ñù±¾¶ÔÍøÂç½øĞĞÑµÁ·£¬²¢°´ÕÕÉÏÃæµÄ¹«Ê½¼ÆËãÃ¿¸öÑù±¾µÄ¦¤ti,t=1,...,T-1|
3.ÑµÁ·p¸öÑù±¾ºó(Ò»´Îbatch)£¬°´ÕÕ¸üĞÂ·½³Ì¶ÔWÓëb½øĞĞ¸üĞÂ.
4.ÖØ¸´²½Öè2¡«3£¬Ö±µ½Îó²îĞ¡ÓÚÉè¶¨µÄãĞÖµ»òÕß´ïµ½Éè¶¨µÄbatch´ÎÊı.
*/
								  ...
								  ccv_convnet_train_param_t params)
{
	...
	// ¶¯Á¿£¨Momentum£©£º²»½ö½ö¿¼ÂÇµ±Ç°Ñù±¾Ëù¼ÆËãµÄÌİ¶È£»
	...
	for (t = 0; t < params.max_epoch; t++)
	{
		...
		for (i = 0; i < aligned_rnum; i++)
		{
			...
			// 3.ÑµÁ·p¸öÑù±¾ºó(Ò»´Îbatch)£¬°´ÕÕ¸üĞÂ·½³Ì¶ÔWÓëb½øĞĞ¸üĞÂ.
			// mini_batchÓÃÓÚSGDÅú´¦ÀíµÄÑù±¾ÊıÄ¿
