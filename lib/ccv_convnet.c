...
/*
ÓÃÖ¸¶¨µÄ²ÎÊý´´½¨Ò»¸öÐÂµÄÉî¶È¾í»ýÍøÂç
Create a new (deep) convolutional network with specified parameters. ccv only 
supports convolutional layer (shared weights), max pooling layer, average 
pooling layer, full connect layer and local response normalization layer.
...
int ccv_convnet_verify(ccv_convnet_t* convnet, int output)
{
	...
	// ÔÚµÚÒ»¸öÈ«Á¬½Ó²ãÖ®ºóµÄËùÓÐ²ã¶¼Ó¦¸ÃÖ»ÊÇÈ«Á¬½Ó²ã
...
#else
static inline void 
_ccv_convnet_convolutional_forward_propagate_fallback(ccv_convnet_layer_t* layer, 
...
static void 
_ccv_convnet_max_pool_forward_propagate(ccv_convnet_layer_t* layer, 
										ccv_dense_matrix_t* a, 
										ccv_dense_matrix_t** b)
{
	int rows, cols, partition;
...
// ÓÃ¾í»ýÍøÂçÀ´°ÑÍ¼Ïñ±àÂë³É½ô´Õ±íÊ¾
/*
...
void ccv_convnet_encode(ccv_convnet_t* convnet, 
						ccv_dense_matrix_t** a, 
						ccv_dense_matrix_t** b, 
						int batch)
{
	...
	// Èç¹û±àÂëµ½Ò»¸ö²»Í¬µÄ¾ØÕóÔò±£´æÉñ¾­»º³åÀïµÄ×îºóÒ»²ã
	...
	// ½ô´Õ±íÊ¾µÄÊä³ö¾ØÕócÊý×é
	convnet->acts[convnet->count - 1] = *b;

	// a: ÊäÈëÍ¼ÏñCÊý×é
	// ´ÓÍ¼ÏñÊäÈë²ãµ½µÚ0²ãÕýÏò´«²¥
...
// ÕÒµ½ÕýÔÚÉ¨ÃèµÄ²ã(ËüÊÇ×îºóÒ»¸ö¾í»ý²ã)
...
static int _ccv_convnet_find_scan(ccv_convnet_t* convnet)
{
	...
	{
		// Èç¹û²ãÀàÐÍÎª¾í»ý²ãÔò·µ»Ø²ãÐòºÅ	
...
batch: The number of input images.
*/
void ccv_convnet_classify(ccv_convnet_t* convnet, 
						  ccv_dense_matrix_t** a, 
						  int symmetric, 
						  ccv_array_t** ranks, 
						  int tops, 
						  int batch)
{
	...
	for (i = 0; i < batch; i++)
	{
		...
		// Èç¹ûÊäÈë¶Ô³ÆÔòÑ­»·2´Î£¬Èç¹û²»¶Ô³ÆÔòÑ­»·1´Î
		for (t = 0; t <= !!symmetric; t++)
		{
			rows = b[0]->rows, cols = b[0]->cols;

			// ´ÓµÚ0²ãÑ­»·µ½×îºóÒ»¸ö¾í»ý²ã
			for (j = 0; j < scan + 1; j++)
			{
				ccv_convnet_layer_t* layer = convnet->layers + j;

				// ¸ù¾Ý²ãÀàÐÍ¼ÆËãÉèÖÃÊä³öÐÐÁÐÊýºÍ·Ö¸î·½Ê½
		...
		// ÏÖÔÚÔÚcÀïÃæÓÐËùÓÐµÄÁË£¬×ö×îºóµÄÈ«Á¬½Ó´«²¥
		...
		for (j = 0; j < tops; j++)
		{
			...
			{
				// ¼ÇÂ¼Ã¿Ò»ÐÐµÄ×î´óÖµmax_valºÍË÷Òýmax_idx	
...
static void 
_ccv_convnet_full_connect_backward_propagate(ccv_convnet_layer_t* layer, 
											 ccv_dense_matrix_t* a, // dloss 
											 ccv_dense_matrix_t* y, 
											 ccv_dense_matrix_t* x, 
											 ccv_dense_matrix_t** b, 
											 ccv_convnet_layer_t* update_params)
{
	// aÊÇÊäÈëÌÝ¶È¾ØÕó(ÓÃÓÚ·´Ïò´«²¥),yÊÇÊä³ö¾ØÕó(ÓÃÓÚÕýÏò´«²¥)
	// xÊÇÊäÈë¾ØÕó(ÓÃÓÚÕýÏò´«²¥),bÊÇÊä³öÌÝ¶È¾ØÕó(ÌÝ¶È,»ò³ÆÎª´«²¥ÌÝ¶ÈÎó²î)
...
static void 
_ccv_convnet_max_pool_backward_propagate(ccv_convnet_layer_t* layer, 
										 ccv_dense_matrix_t* a, 
										 ccv_dense_matrix_t* n, 
...
static void 
_ccv_convnet_average_pool_backward_propagate(ccv_convnet_layer_t* layer, 
											 ccv_dense_matrix_t* a, 
											 ccv_dense_matrix_t* m, 
											 ccv_dense_matrix_t** b)
{
							...
							ccv_convnet_t* update_params)
{
	...
	// ×îºóÒ»²ã±ØÐëÊÇÈ«Á¬½Ó²ãÒÔÉú³Ésoftmax½á¹û
	assert(layer->type == CCV_CONVNET_FULL_CONNECT); // the last layer has too be a full connect one to generate softmax result
	_ccv_convnet_full_connect_backward_propagate(layer, 
												 dloss, 
												 convnet->acts[convnet->count - 1], 
												 convnet->acts[convnet->count - 2], 
												 convnet->count - 1 > 0 ? update_params->acts + convnet->count - 2 : 0, 
												 update_params->layers + convnet->count - 1);

	// ´Óµ¹ÊýµÚ¶þ²ãÑ­»·µ½µÚÒ»²ã
	for (i = convnet->count - 2; i >= 0; i--)
	{
		...
		switch (layer->type)
		{
			case CCV_CONVNET_CONVOLUTIONAL:
				...
				_ccv_convnet_convolutional_backward_propagate(layer, 
															  update_params->acts[i], 
															  convnet->acts[i], 
															  i > 0 ? convnet->acts[i - 1] : a, 
															  i > 0 ? update_params->acts + i - 1 : 0, 
															  update_params->layers + i);
			...
			case CCV_CONVNET_LOCAL_RESPONSE_NORM:
				...
				_ccv_convnet_rnorm_backward_propagate(layer, 
													  update_params->acts[i], 
													  convnet->acts[i], 
													  i > 0 ? convnet->acts[i - 1] : a, 
													  convnet->denoms[i], 
			...
			case CCV_CONVNET_MAX_POOL:
				...
				_ccv_convnet_max_pool_backward_propagate(layer, 
														 update_params->acts[i], 
														 convnet->acts[i], 
														 i > 0 ? convnet->acts[i - 1] : a, 
														 i > 0 ? update_params->acts + i - 1 : 0);
			...
			case CCV_CONVNET_AVERAGE_POOL:
				...
				_ccv_convnet_average_pool_backward_propagate(layer, 
															 update_params->acts[i], 
															 i > 0 ? convnet->acts[i - 1] : a, 
															 i > 0 ? update_params->acts + i - 1 : 0);
	...
	for (i = 0; i < convnet->count; i++)
	{
		switch (update_params->layers[i].type)
		{
			case CCV_CONVNET_CONVOLUTIONAL:
			{
				...
				{
					vw[j] = 
						layer_params[i].w.momentum * vw[j] - 
						layer_params[i].w.decay * layer_params[i].w.learn_rate * w[j] + 
						learn_rate * dw[j];
			...
			case CCV_CONVNET_FULL_CONNECT:
			{
				...
				for (j = 0; j < convnet->layers[i].wnum; j++)
				{
					// ¸üÐÂÈ¨ÖØ:ÕâÀïÓÃ×î¼òµ¥µÄ·½·¨À´¸üÐÂ£¬¼´ËùÓÐ²ÎÊý¶¼W = W - learn_rate * gradient
					vw[j] = 
						layer_params[i].w.momentum * vw[j] 
					  - layer_params[i].w.decay * layer_params[i].w.learn_rate * w[j] 
					  + learn_rate * dw[j];
				...
				// °´µÚi²ãµÄÊä³ö½ÚµãÊýÄ¿À´Ñ­»·
				for (j = 0; j < convnet->layers[i].net.full_connect.count; j++)
				{
					vbias[j] 
						= layer_params[i].bias.momentum * vbias[j] 
						- layer_params[i].bias.decay * layer_params[i].bias.learn_rate * bias[j]
						+ learn_rate * dbias[j];
...
/*
¼¤»îº¯ÊýµÄÑ¡Ôñ 
ÓÐ¶à¸ö¼¤»îº¯Êý¿É¹©Ñ¡Ôñ£¬¾ßÌåÑ¡ÔñÄÄÒ»¸öÔòÐèÒª¸ù¾ÝÊµ¼ÊµÄÎÊÌâ½øÐÐÑ¡È¡. ÆäÊµ¼¤»îº¯Êý
Ëµ²»ÉÏË­±ÈË­ºÃ£¬¶ÔÓÚÒþº¬²ãµÄ¼¤»îº¯Êý£¬¿ÉÒÔÑ¡Ôñsigmoid¡¢tanh¡¢reluµÈ¶àÖÖ¼¤»îº¯Êý. 
¶ø¶ÔÓÚÊä³ö²ãµÄ¼¤»îº¯Êý£¬ÔòÊÇÓÐÒ»¶¨Ñ¡ÔñµÄ£ºÈç¹ûÊÇÁ½Àà·ÖÀàÎÊÌâ£¬Êä³ö²ãÖ»ÓÐÒ»¸öÉñ¾­
Ôª£¬ÄÇÃ´Ñ¡Ôñsigmoid(logistic); Èç¹ûÊÇnÀà·ÖÀàÎÊÌâ£¬ÔòÊä³ö²ãÓÐn(»òn-1)¸öÉñ¾­Ôª£¬ÄÇ
Ã´Ñ¡Ôñsoftmax£»Èç¹ûÊÇ»Ø¹éÎÊÌâ£¬ÄÇÃ´Ñ¡ÔñÏßÐÔ¼¤»îº¯Êý.

softmaxÄ£ÐÍÊÇlogisticÄ£ÐÍÔÚ¶à·ÖÀàÎÊÌâÉÏµÄÍÆ¹ã£¬ logistic »Ø¹éÊÇÕë¶Ô¶þ·ÖÀàÎÊÌâµÄ
Èç¹ûÄ³Ò»¸özj´ó¹ýÆäËûz,ÄÇÕâ¸öÓ³ÉäµÄ·ÖÁ¿¾Í±Æ½üÓÚ1,ÆäËû¾Í±Æ½üÓÚ0£¬Ö÷ÒªÓ¦ÓÃ¾ÍÊÇ¶à·Ö
Àà£¬sigmoidº¯ÊýÖ»ÄÜ·ÖÁ½Àà£¬¶øsoftmaxÄÜ·Ö¶àÀà£¬softmaxÊÇsigmoidµÄÀ©Õ¹¡
Èç¹ûÀà±ðÖ®¼äÊÇ»¥³âµÄ£¬ÊÊºÏÓÃsoftmax£»Èç¹ûÀà±ðÖ®¼äÔÊÐíÖØµþ£¬Ó¦¸ÃÑ¡Ôñk¸ölogistic·ÖÀàÆ÷
*/
										 ...
										 int type)
{
	...
	for (i = 0; i < a->rows * a->cols * ch; i++)
	{
		// ÕâÒ»Ïî¶Ô¸ÅÂÊ·Ö²¼½øÐÐ¹éÒ»»¯£¬Ê¹µÃËùÓÐ¸ÅÂÊÖ®ºÍÎª 1 
		bptr[i] *= tt;
	}
}
...
/*
categorizeds: ÓÃÀ´ÑµÁ·µÄ´ø·ÖÀàÐÅÏ¢µÄÍ¼Ïñ×é
tests: ÓÃÀ´ÑéÖ¤µÄ´ø·ÖÀàÐÅÏ¢µÄÍ¼Ïñ×é
...
1.Ê×ÏÈÊ¹ÓÃËæ»úº¯Êý¶ÔÃ¿Ò»²ã¼äµÄÁ¬½ÓÈ¨Öµ¾ØÕóºÍÆ«ÖÃÏòÁ¿½øÐÐËæ»ú³õÊ¼»¯.
2.ÒÀ´ÎÊ¹ÓÃÒ»¸öÑµÁ·Ñù±¾¶ÔÍøÂç½øÐÐÑµÁ·£¬²¢°´ÕÕÉÏÃæµÄ¹«Ê½¼ÆËãÃ¿¸öÑù±¾µÄ¦¤ti,t=1,...,T-1|
3.ÑµÁ·p¸öÑù±¾ºó(Ò»´Îbatch)£¬°´ÕÕ¸üÐÂ·½³Ì¶ÔWÓëb½øÐÐ¸üÐÂ.
4.ÖØ¸´²½Öè2¡«3£¬Ö±µ½Îó²îÐ¡ÓÚÉè¶¨µÄãÐÖµ»òÕß´ïµ½Éè¶¨µÄbatch´ÎÊý.
*/
void ccv_convnet_supervised_train(ccv_convnet_t* convnet, 
								  ccv_array_t* categorizeds, 
								  ccv_array_t* tests, 
								  const char* filename, 
								  ccv_convnet_train_param_t params)
{
	...
	// ×îºóÒ»²ã±ØÐëÊÇÈ«Á¬½Ó£¬Òò´ËÎÒÃÇ°ÑËüÓÃ×÷softmax²ã
	...
	for (t = 0; t < params.max_epoch; t++)
	{
		...
		for (i = 0; i < aligned_rnum; i++)
		{
			...
			// 3.ÑµÁ·p¸öÑù±¾ºó(Ò»´Îbatch)£¬°´ÕÕ¸üÐÂ·½³Ì¶ÔWÓëb½øÐÐ¸üÐÂ.
			// mini_batchÓÃÓÚSGDÅú´¦ÀíµÄÑù±¾ÊýÄ¿
			if ((i + 1) % params.mini_batch == 0)
			{
				...
				// ¸üÐÂÈ¨ÖØ,Êä³öµ½convnet->layers[i]ºÍmomentum->layers[i]
				// update weights
				...
				// ÊÍ·ÅÒ»¸ö¸ø¶¨µÄ¾í»ýÍøÂçµÄÁÙÊ±×ÊÔ´
				// Ñ¹Ëõ¾í»ýÍøÂçÒÔ±ÜÃâ³öÏÖÈÎºÎ±ä¾ÉµÄÁÙÊ±×ÊÔ´
		...
		for (i = 0; i < tests->rnum; i++)
		{
			...
			// ½«µ±Ç°²âÊÔÍ¼Ïñ·Åµ½¾í»ýÍøÂçÀïÃæ×÷·ÖÀà£¬Êä³öÀà±ðÐÅÏ¢µ½c
		...
		{
			// ½«ÒÑ·ÃÎÊµÄ²¿·ÖÖØÐÂÏ´ÅÆ²¢ÇÒ½«Ê£ÓàµÄ²¿·ÖÒÆ¶¯µ½Æðµã
