...
inline static void 
ccv_convnet_make_output(ccv_convnet_layer_t* layer, 
						int input_rows, 
						int input_cols, 
						int* rows, 
						int* cols, 
						int* partition) // 输入矩阵的分割数目(必须由通道数来分割)
{
	...
	case CCV_CONVNET_CONVOLUTIONAL:
		...
		*rows = (input_rows + layer->net.convolutional.border * 2 
				 - layer->net.convolutional.rows + layer->net.convolutional.strides - 1) 
			  / layer->net.convolutional.strides + 1;
		
		*cols = (input_cols + layer->net.convolutional.border * 2 
			     - layer->net.convolutional.cols + layer->net.convolutional.strides - 1) 
			  / layer->net.convolutional.strides + 1;
